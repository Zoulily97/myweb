            ...
            <% for (int i = 0; i < list.Count; i++)
                {
                    ...
                    %>
                <tr><td><% =list[i].Id%></td><td><%=list[i].Username %></td><td><%=list[i].Password %></td><td><%=list[i].Name %></td><td><% =list[i].Adress%></td><td><% =sex%></td><td><% =list[i].Mobile%></td><td><% =list[i].Email%></td><td><% =list[i].Qq%></td><td><% =state%></td><td><% =list[i].Addate%></td><td><a href="index.aspx?id=<%=list[i].Id %>" >删除</a><a href="user_edit.aspx?id=<%=list[i].Id %>" >编辑</a></td></tr>
